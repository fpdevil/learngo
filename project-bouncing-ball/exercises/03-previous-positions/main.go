...
func main() {
	...
	var (
		...
		// using the defined custom velocities now
		vx, vy = cx, cy
	...
	for i := 0; i < maxFrames; i++ {
		...
		// updating the position to include the velocity for unit time as well
		if px <= 0 || px >= width-vx {
			vx *= -1
		}
		...
		// --- This is replaced with board[x][y] = false ---
		// after the buf = append(buf, cell, ' ')
		//
