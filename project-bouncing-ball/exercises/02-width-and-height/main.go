...
func main() {
	// setting width and height of the board using os terminal
	...
	// using screen package instead to get width and height
	width, height := screen.Size()

	bufLen := (width*2 + 1) * height
	fmt.Printf("width: %d, height: %d, buflength: %d\n", width, height, bufLen)
	...
	// get the width of the ball emoji
	ballWidth := runewidth.RuneWidth(cellBall)

	// adjusting the ball emoji span
	width /= ballWidth
	height--
