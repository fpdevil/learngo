...
func main() {
	...
	// create the board
	//------------------------------------------------------
	// using a single dimension slice rather thana multi
	// dimensional slice for the board
	board := make([]bool, width*height)
	//------------------------------------------------------
	// board := make([][]bool, width)
	...
	for i := 0; i < maxFrames; i++ {
		...
		// now the board being single dimension, we have to fetch
		// the position(s) of the ball based on width defined
		// current position of ball
		currPos := py*width + px

		// previous position of ball
		prevPos := ppy*width + ppx

		// remove the previous ball and put the new ball
		// board[px][py], board[ppx][ppy] = true, false
		board[currPos], board[prevPos] = true, false
		...
		// for each value verticall, we get the horizontal row
		for y := 0; y < height; y++ {
			for x := 0; x < width; x++ {
				cell = cellEmpty
				if board[y*width+x] {
