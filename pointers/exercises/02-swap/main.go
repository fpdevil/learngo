...
func main() {
	fmt.Println("---- 26-pointers/02-swap ----")
	fmt.Println("     EXERCISE: Swap          ")

	a, b := 4.0, 9.0
	fmt.Printf("Before Swap : a: %g, b: %g\n", a, b)
	swapVals(&a, &b)
	fmt.Printf("After Swap  : a: %g, b: %g\n", a, b)

	pa, pb := &a, &b
	fmt.Printf("Before Swap : pa: %p, pb: %p\n", pa, pb)
	fmt.Printf("Before Swap : pa: %g, pb: %g\n", *pa, *pb)

	// now swap the addresses
	pa, pb = swapAddrs(pa, pb)
	fmt.Printf("After Swap  : pa: %p, pb: %p\n", pa, pb)
	fmt.Printf("After Swap  : pa: %g, pb: %g\n", *pa, *pb)
}

func swapVals(a, b *float64) {
	*a, *b = *b, *a
}

func swapAddrs(a, b *float64) (*float64, *float64) {
	return b, a
}
